...
namespace MagentoScanner.Core
{
    public static class VersionIdentifier
    {
        public static async Task TryToDetectVersion(TargetOptions targetOptions, HttpClient client)
        {
            ...
            {
                HttpResponseMessage result2 = await client.GetAsync(string.Concat(result.RequestMessage.RequestUri.ToString(), "magento_version"), HttpCompletionOption.ResponseHeadersRead);
            ...
            if (!versionIdentified)
            {
                ...
                {
                    result = await client.GetAsync(new Uri(string.Concat(Helper.AddSlash(targetOptions), jToken.Key)), HttpCompletionOption.ResponseHeadersRead);
                    if (result.IsSuccessStatusCode)
                    {
                        string fileHash = HashCalculator.CalculateMD5(await result.Content.ReadAsByteArrayAsync()).ToLower();
                        if (jToken.Value.SelectToken(fileHash) != null)
                        {
                            Logger.Log(Importance.Info, "Magento version used: " + jToken.Value.SelectToken(fileHash).Value<string>(), ConsoleColor.Green);
                            versionIdentified = true;
                            break;
                        }
                    }
    ...
    Path
    .Combine(AppDomain.CurrentDomain.BaseDirectory.CurrentProjectFolder(), "..\\Resources\\version_hashes.json");
